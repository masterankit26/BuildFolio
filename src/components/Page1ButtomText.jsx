
import React from 'react';

const Page1ButtomText = () => {
  return (
    <div className='absolute bottom-2 left-6 sm:left-10 px-4 py-2 rounded-md shadow-md'>
      <div className='text-white text-sm sm:text-base leading-relaxed font-semibold text-left'>
        <h2 className='text-3xl font-serif text-green-200'><span className='text-blue-300'>Ankit Shaw |</span>KGEC IT '27</h2>
        <h3 className='text-3xl font-serif text-amber-50'>JavaScript |<span className='text-amber-300'>Python</span></h3>
      </div>
    </div>
  );
};

export default Page1ButtomText;
